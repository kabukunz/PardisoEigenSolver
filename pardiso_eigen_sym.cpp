...
/* -------------------------------------------------------------------- */
    ...
    tripletList.reserve(pardiso.a.size());

    for (size_t k = 0; k < pardiso.a.size(); k++)
    {
       int a = pardiso.a.at(k);
       int i = pardiso.ia.at(k);
       int j = pardiso.ja.at(k);

       tripletList.push_back(T(i, j, a));
    }
    SparseMatrix<double> A;
    A.resize(pardiso.ia.size(), pardiso.ja.size());
    A.setFromTriplets(tripletList.begin(), tripletList.end());
    A.makeCompressed();

    // fill b
    Eigen::VectorXd b;
    b.resize(pardiso_tu.size());
    for (size_t i = 0; i < pardiso_tu.size(); i++)
       b(i) = pardiso_tu.at(i);
